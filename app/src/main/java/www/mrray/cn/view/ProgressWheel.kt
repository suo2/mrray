...
class ProgressWheel : View {
    ...
    private var mCountText: String? = "" // TODO: use a default from R.string...
    ...
    private val transformRatio :Float = 360/100f //转化为圆的比例
    ...
    private fun init(attrs: AttributeSet?, defStyle: Int) {
        ...
        mPercentage = a.getFloat(R.styleable.ProgressWheel_percentage, mPercentage) * transformRatio //转换为圆的比例
    ...
    fun setStepCountText(countText: String?) {
        mCountText = if(countText.isNullOrEmpty()) "0%" else "$countText%"
        invalidate()
    }
    ...
    fun setPercentage(per: String?) {
        ...
        startAnimation(perTemp * transformRatio)
    }

    private fun startAnimation(per: Float) {
        ValueAnimator.ofFloat()
        val diff = per - mPercentage
        ValueAnimator()
        val valueAnimator = ValueAnimator
                .ofFloat(mPercentage, (mPercentage + diff))
                .setDuration(1000)
        valueAnimator.addUpdateListener { animation ->
            mPercentage = (animation.animatedValue as Float)
            invalidate()
        }
        valueAnimator.start()
    }
