...
import kotlinx.android.synthetic.main.fragment_home_gp.*
...
import www.mrray.cn.module.gp.activity.HighProjectActivity
import www.mrray.cn.module.gp.adapter.HighCustomerAdapter
import www.mrray.cn.module.gp.adapter.HighProjectAdapter
 ...
 *@desc: GP首页
 */
class GpHomeFragment : BaseViewModelFragment<GpHomeViewModel,GpHomeRepository>() {
    private lateinit var mPageRequestBody: PageRequestBody
    private lateinit var projectList: ArrayList<HighProjectModel>
    private lateinit var customerList: ArrayList<HighCustomerModel>
    private lateinit var newsInfoList: ArrayList<NewsInfoModel>
    private lateinit var mHighProjectAdapter: HighProjectAdapter
    ...
    companion object {
        fun newInstance() = GpHomeFragment()
    }

    override fun getLayoutId(): Int {
        return R.layout.fragment_home_gp
    }
    ...
    private fun requestData(){
        ...
        mViewModel.getHead()
        mViewModel.getProjectPageQueryHome(mPageRequestBody.getMap())
        mViewModel.getInvestorPageQueryHome(mPageRequestBody.getMap())
    }
    ...
    private fun initData() {
        high_project_more.setOnClickListener { startActivity<HighProjectActivity>() }
        high_customer_more.setOnClickListener { startActivity<HighCustomActivity>() }

        //初始化数据
        highProjectRecyclerView.layoutManager = object : LinearLayoutManager(context) {
            override fun canScrollVertically() = false
        }
        highProjectRecyclerView.addItemDecoration(SpaceItemDecoration(20,3))
    ...
    override fun setUpViewModel() {
        ...
        mViewModel.mHighProjectHomeModel.observe(this, Observer {
            projectList = (it as PageBaseModel<HighProjectModel>).content as ArrayList<HighProjectModel>
            mHighProjectAdapter = HighProjectAdapter(activity!!, projectList)
            highProjectRecyclerView.adapter = mHighProjectAdapter
            mSwipeRefreshLayout.isRefreshing = false
        })

        mViewModel.mHighCustomerHomeModel.observe(this, Observer {
            customerList = (it as PageBaseModel<HighCustomerModel>).content as ArrayList<HighCustomerModel>
            mHighCustomerAdapter = HighCustomerAdapter(activity!!, customerList,true)
