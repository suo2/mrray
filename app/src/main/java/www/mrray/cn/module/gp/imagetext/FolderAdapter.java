        ...
        if (holder != null) {
            if(i == 0){
                ...
                if(mFolders.size()>0){
                    ...
                    Glide.with(mContext)
                            .load(new File(f.getCover().getPath()))
                            .apply(new RequestOptions().placeholder(R.mipmap.default_error).error(R.mipmap.default_error).override(mImageSize, mImageSize)
                                    .centerCrop())
                            .into(holder.cover);
                }
            }else {
